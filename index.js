...
if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
    console.error("❌ FATAL ERROR: Variabel TELEGRAM_BOT_TOKEN atau TELEGRAM_CHAT_ID tidak ditemukan di file .env!");
    process.exit(1);
}
...
const botState = {
    ...
    jobQueue: new Map(),
    config: { 
        batch: parseInt(process.env.DEFAULT_BATCH_SIZE) || 25, 
        delay: parseInt(process.env.DEFAULT_DELAY_MS) || 7000 
    }
};
...
console.log('⚡️ Memulai Engine Bot Telegram (Env Mode)...');
...
teleBot.on('message', async (msg) => {
    ...
    if (botState.waitingForInput) {
        ...
        if (!/^\d{8,16}$/.test(phoneNumber)) {
            ...
            return teleBot.sendMessage(msg.chat.id, "❌ *Validasi Gagal!*\nFormat nomor tidak valid. Pastikan panjang nomor sesuai standar dan diawali kode negara.", { parse_mode: 'Markdown', reply_markup: errKeyboard });
        }
...
teleBot.on('document', async (msg) => {
    ...
    try {
        ...
        if (uniqueNumbers.length === 0) {
            return teleBot.sendMessage(msg.chat.id, '❌ *Data Kosong atau Tidak Valid!*\nPastikan file .txt berisi nomor beserta kode negara.', { parse_mode: 'Markdown' });
        }
...
async function processBulkCheck(numbers, config, chatId, msgId) {
    ...
    // Fix EFATAL Buffer Telegram (Memberikan header contentType eksplisit)
    const fileOpts = { contentType: 'text/plain' };

    if (countPersonal > 0) {
        teleBot.sendDocument(chatId, Buffer.from(resultPersonal, 'utf-8'), {}, { ...fileOpts, filename: `WA_Personal_Global_${Date.now()}.txt` });
    }
    if (countBisnis > 0) {
        teleBot.sendDocument(chatId, Buffer.from(resultBisnis, 'utf-8'), {}, { ...fileOpts, filename: `WA_Bisnis_Global_${Date.now()}.txt` });
    }
    if (countTidakTerdaftar > 0) {
        teleBot.sendDocument(chatId, Buffer.from(resultTidakTerdaftar, 'utf-8'), {}, { ...fileOpts, filename: `WA_Mati_Global_${Date.now()}.txt` });
    }
}
